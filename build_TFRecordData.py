...
tf.app.flags.DEFINE_string('output_directory_validation', './data/TFRecords/validation',
                           'Output data directory - validation')
tf.app.flags.DEFINE_string('output_directory_test', './data/TFRecords/test',
                           'Output data directory - test')



tf.app.flags.DEFINE_integer('train_shards', 4,
                            'Number of shards in training TFRecord files.') #tyle bedzie plikow TFRecord - z tego wynika ile zdjec zapisujemy w jednym pliku TFRecord
...
def _process_image_files_batch(coder, thread_index, ranges, name, filenames,
    ...
    for s in range(num_shards_per_batch):
        ...
        #moja wstawka    !!!!!!!!!!!!!!!
        if name =='train':
            output_file = os.path.join(FLAGS.output_directory_train, output_filename)
        elif name == 'test':
            output_file = os.path.join(FLAGS.output_directory_test, output_filename)
        else:
            output_file = os.path.join(FLAGS.output_directory_validation, output_filename)
        #koniec wstawski
